...
## Breaking Change for dbt Core < 1.9 (not relevant to Quickstart)
> *Note: This is not relevant to Fivetran Quickstart users.*

Migrated `freshness` from a top-level source property to a source `config` in alignment with [recent updates](https://github.com/dbt-labs/dbt-core/issues/11506) from dbt Core. This will resolve the following deprecation warning that users running dbt >= 1.9 may have received:
...
**IMPORTANT:** Users running dbt Core < 1.9 will not be able to utilize freshness tests in this release and any subsequent releases, as older versions of dbt will not recognize freshness as a source `config` and therefore not run the tests.

If you are using dbt Core < 1.9 and want to continue running TikTok Ads freshness tests, please elect **one** of the following options:
  1. (Recommended) Upgrade to dbt Core >= 1.9.
  2. Do not upgrade your installed version of `tiktok_ads_source` and pin your package dependency on v0.8.0.
