# [DEPRECATED] Tiktok Ads Source dbt Package

> _This package has been archived and is no longer maintained. Use [`dbt_tiktok_ads`](https://github.com/fivetran/dbt_tiktok_ads) instead, which now contains all models previously found here._
...
- Materializes [Tiktok Ads staging tables](https://fivetran.github.io/dbt_tiktok_ads_source/#!/overview/tiktok-ads-source/models/?g_v=1) which leverage data in the format described by [this ERD](https://fivetran.com/docs/applications/tiktok-ads/#schemainformation). These staging tables clean, test, and prepare your Tiktok Ads data from [Fivetran's connector](https://fivetran.com/docs/applications/tiktok-ads) for analysis by doing the following:
  ...
  - Adds freshness tests to source data.
    > dbt Core >= 1.9.6 is required to run freshness tests out of the box. See other options [here](https://github.com/fivetran/dbt_tiktok_ads_source/blob/main/CHANGELOG.md#breaking-change-for-dbt-core--196).
...
This package leverages the `campaign_country_report` table to help report on ad and campaign performance by country/geographic region. However, if you are not actively syncing this report from your TikTok Ads connection, you may disable the transformations for the `campaign_country_report` by adding the following variable configuration to your root `dbt_project.yml` file:
```yml
vars:
